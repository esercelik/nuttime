...
@switch($section['type'])
    @case('banner')
        @if(!empty($section['desktop_image']))
            <section class="brand-moment" @if(!empty($section['background_color'])) style="background-color: {{ $section['background_color'] }}" @endif>
                ...
                <div class="container brand-moment__copy"><p class="kicker"><x-safe-rich-text :value="$section['eyebrow']" /></p><p><x-safe-rich-text :value="$section['title']" /></p>@if($buttonUrl)<a class="arrow-link arrow-link--light" href="{{ $buttonUrl }}"><x-safe-rich-text :value="$section['button_label'] ?: __('site.actions.learn_more')" /> <span>↗</span></a>@endif</div>
            </section>
        @endif
        @break

    @case('intro')
        <section class="home-intro"><div class="container home-intro__inner"><p class="kicker"><x-safe-rich-text :value="$section['eyebrow']" /></p><p><x-safe-rich-text :value="$section['title']" /></p>@if($section['description'])<p class="section-copy"><x-safe-rich-text :value="$section['description']" /></p>@endif @if($buttonUrl)<a class="arrow-link" href="{{ $buttonUrl }}"><x-safe-rich-text :value="$section['button_label'] ?: __('site.actions.learn_more')" /> <span>↗</span></a>@endif</div></section>
        @break
    ...
    @case('social')
        ...
        @if($socialUrl)<section class="social-callout"><div class="container"><p class="kicker"><x-safe-rich-text :value="$section['eyebrow']" /></p><h2><x-safe-rich-text :value="$section['title']" /></h2><a class="arrow-link arrow-link--light" href="{{ $socialUrl }}" target="_blank" rel="noopener noreferrer"><x-safe-rich-text :value="$section['button_label'] ?: ucfirst($network)" /> <span>↗</span></a></div></section>@endif
        @break
