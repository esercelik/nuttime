...
<a {{ $attributes->class('catalog-card catalog-card--'.$variant) }} href="{{ app(\App\Support\LocalizedUrl::class)->route('product', null, ['slug' => $product['slug']]) }}">
    ...
    <div class="catalog-card__info">
        <p><x-safe-rich-text :value="$product['category'] ?? 'Nuttime'" /></p>
        <h3><x-safe-rich-text :value="$product['name'] ?? 'Nuttime'" /></h3>
        @if(!empty($product['description']))
            <span class="catalog-card__summary"><x-safe-rich-text :value="\Illuminate\Support\Str::limit($product['description'], 94)" /></span>
        @endif
