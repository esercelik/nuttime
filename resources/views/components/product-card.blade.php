...
<a {{ $attributes->class('catalog-card catalog-card--'.$variant) }} href="{{ app(\App\Support\LocalizedUrl::class)->route('product', null, ['slug' => $product['slug']]) }}">
    ...
    @endif
    <div class="catalog-card__info"><p>{{ $product['category'] ?? 'Nuttime' }}</p><h3>{{ $product['name'] ?? 'Nuttime' }}</h3>@if(!empty($product['description']))<span class="catalog-card__summary">{{ \Illuminate\Support\Str::limit($product['description'], 94) }}</span>@endif<span class="catalog-card__action">{{ __('site.actions.view_product') }} <b>↗</b></span></div>
</a>
